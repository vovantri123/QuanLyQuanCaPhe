	...
	BEGIN TRY
		...
		BEGIN
			IF (EXISTS (SELECT 1 FROM ThucHien TH
						JOIN CaLamViec CLV ON TH.MaCa = CLV.MaCa
						WHERE TH.MaNV = @MaNV 
						AND (CLV.TenCa = N'Sáng' OR CLV.TenCa = N'Chiều')  -- Kiểm tra xem nhân viên đã làm FulltimeAM chưa
						AND CLV.Ngay = @Ngay))
			BEGIN
				RAISERROR (N'Nhân viên đã làm ca Sáng không thể đăng ki ca FulltimeAM .', 16, 1); 
				RETURN;
			END
		END
		...
		BEGIN
			IF (EXISTS (SELECT 1 FROM ThucHien TH
						JOIN CaLamViec CLV ON TH.MaCa = CLV.MaCa
						WHERE TH.MaNV = @MaNV 
						AND (CLV.TenCa = N'Tối' OR CLV.TenCa = N'Chiều')  -- Kiểm tra xem nhân viên đã làm FulltimeAM chưa
						AND CLV.Ngay = @Ngay))
			BEGIN
				RAISERROR (N'Nhân viên đã làm ca Tối không thể đăng ki ca FulltimePM .', 16, 1); 
				RETURN;
			END
		END
